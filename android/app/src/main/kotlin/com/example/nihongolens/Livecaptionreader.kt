...
class LiveCaptionReader : AccessibilityService() {
    ...
    private fun handleLcGoneOrAbsent(): String? {
        if (lcVisible) {
            ...
            lcVisible = false
            lcGoneMs  = 0L
            // FIX: previously reset lastRawFull/lastEnqueued/lastSentText/
            // lastSubmitTotalWords/lastEnqueuedText here unconditionally.
            // Live Caption's transcript is CUMULATIVE — it doesn't shrink —
            // so after a brief gone/reappear cycle (video briefly paused, a
            // transient accessibility-tree hiccup, app switch, etc.) the
            // transcript almost always still contains everything already
            // submitted. Wiping lastSubmitTotalWords made schedule()'s tail
            // extraction (line ~582: `if (lastSubmitTotalWords <= 0) return
            // fullText`) treat the ENTIRE transcript as brand new — silently
            // re-submitting and re-speaking everything already translated,
            // from the start, every time LC blinked. That's the exact "it
            // starts reading from the beginning again" bug.
            // Only abandon the in-flight, never-to-complete fragment here —
            // the genuine-new-session case (LC restarts with an unrelated,
            // shorter transcript) is detected separately in readWindowFound's
            // "LC appeared" branch by comparing word counts, and reset there
            // only if that specific signal fires.
            pendingJob?.cancel(); pendingJob = null
            sentenceTimerJob?.cancel(); sentenceTimerJob = null
            sentenceBuffer = ""; lastBufferEnqueued = ""
            CaptionLogger.log(TAG, "LC gone (grace ${goneForMs}ms — confirmed, tracking preserved)")
    ...
    private fun readWindowFound(root: AccessibilityNodeInfo): String? {
        ...
        if (!lcVisible) {
            lcVisible = true
            // FIX: previously always reset lastRawFull/lastEnqueuedSents/
            // dedup here unconditionally, which caused re-translation of
            // the entire transcript from the start on every LC blink (see
            // the matching comment in handleLcGoneOrAbsent above for the
            // full explanation). Now: only reset if this is genuinely a
            // NEW/different session — detected by the reappearing
            // transcript being SHORTER (in words) than what we'd already
            // submitted. Live Caption's transcript only ever grows within
            // one continuous session, so a shorter transcript means it
            // restarted (new video/scene) rather than just blinked.
            val reappearedWords = wc(full)
            if (reappearedWords < lastSubmitTotalWords) {
                lastRawFull = ""
                lastEnqueuedSents.clear()
                lastEnqueuedText = ""
                lastSubmitTotalWords = 0
                lastSubmitMs = 0L
                resetHindiDedup()  // new scene — don't skip new speaker's first sentence
                CaptionLogger.log(TAG, "LC appeared (new session, transcript reset: ${reappearedWords}w < ${lastSubmitTotalWords}w) '${full.take(60)}'")
            } else {
                CaptionLogger.log(TAG, "LC appeared (same session continues, ${reappearedWords}w) '${full.take(60)}'")
            }
        }
